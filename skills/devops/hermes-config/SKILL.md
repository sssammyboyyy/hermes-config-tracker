...
## Secrets & the write_file `***` Problem

**Critical tool-usage constraint**: The `write_file` tool (and inline Python via `terminal`) interprets three consecutive asterisks (`***`) as markdown formatting. Any string containing `***` — which includes Telegram bot tokens, API keys with base64 segments, etc. — will cause parse errors or silent truncation.

Full details and edge cases: `references/triple-asterisk-escaping.md`
