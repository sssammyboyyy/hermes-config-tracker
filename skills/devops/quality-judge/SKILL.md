...
### 6. Deploy Verification (1-10) — NEW, MANDATORY
- **All assets HTTP 200**: Every image, CSS, JS file on the live URL returns 200
- **All assets are correct MIME type**: `file` command confirms PNG/JPEG (not HTML pages)
- **All JS functions present**: `curl -s URL | grep -c 'function NAME'` > 0 for every expected function
- **No console errors**: JS syntax is valid (balanced braces/parens verified)
- **iframe src resolves**: Mockup URL in report iframe returns 200

**Minimum score: 10/10 to ship** — Any broken asset or missing function fails the entire deliverable.

### 7. Problem→Solution Narrative (1-10) — NEW, MANDATORY
- **Pain points visible**: The deliverable shows the client's exact problems (e.g., "0% review response rate", "3.6★ dragging down brand")
- **Solution demonstrated**: The mockup shows JCE Media's solution to each pain point (e.g., AI chat widget, auto-response, smart routing)
- **Transformation quantified**: Before/after metrics are clear (e.g., "From 3,000 reviews to 10,000")
- **Client-specific**: Impossible to mistake for a generic template — uses real names, real reviews, real data
- **Survives criticism**: A skeptical client couldn't say "this is generic" or "you just changed the colors"

**Minimum score: 9/10 to ship** — This is the main sales weapon. It must be bulletproof.
...
5. **Deploy verification is mandatory** — run the full curl checklist before declaring done
6. **Problem→Solution narrative is the whole point** — without it, the deliverable is just a pretty report
7. **When in doubt, escalate to Samuel** — better to ask than to ship garbage
